...
const $k7QOs$reactdom__namespace = /* @__PURE__ */ _interopNamespaceDefault($k7QOs$reactdom);
const arrow = (options) => {
...
function useFloating(options) {
  ...
  const update = React__namespace.useCallback(() => {
    ...
    floatingUi_dom.computePosition(referenceRef.current, floatingRef.current, config).then((data2) => {
      const fullData = {
        ...data2,
        isPositioned: true
      };
      if (isMountedRef.current && !deepEqual(dataRef.current, fullData)) {
        ...
        $k7QOs$reactdom__namespace.flushSync(() => {
          setData(fullData);
        });
      }
    });
